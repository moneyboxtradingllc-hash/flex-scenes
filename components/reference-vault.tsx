...
export function ReferenceVault({ data, character, refresh, select, back, createCharacter, createWithReferences, onCharacterChange }: {
  ...
  return <main className="reference-vault" data-testid="reference-vault">
    ...
    <section className="reference-vault-toolbar"><button className="reference-primary" onClick={() => setUploadOpen(true)}>＋ Add References</button><button aria-pressed={selectMode} onClick={() => { setSelectMode(!selectMode); setSelected([]); }}>{selectMode ? "Done" : "Select"}</button></section>
    ...
    {filtered.length === 0 ? <section className="reference-vault-empty"><h2>No {referenceCategories.find((item) => item.key === category)?.label ?? "Face"} references yet</h2><p>Add one or more new images or videos in this category.</p><button onClick={() => setUploadOpen(true)}>Add References</button></section> : <section className="reference-vault-grid" aria-label={`${currentCharacter.name} ${category} references`}>{filtered.map(({ ref, asset }) => <article key={`${ref.mediaId}:${ref.role}`} className={!ref.active ? "is-inactive" : ""}>
