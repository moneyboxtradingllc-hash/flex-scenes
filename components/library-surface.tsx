...
export function PremiumLibrary({
  ...
  return <section className="library-vault" aria-labelledby="library-title">
    ...
    <div className="library-characters" aria-label="Filter by character"><button className={`library-all-characters ${!characterId ? "active" : ""}`} onClick={() => setCharacterId("")} aria-pressed={!characterId}>All Characters</button>{data.characters.map((entry) => { const count = data.media.filter((asset) => asset.characterId === entry.id).length; return <button key={entry.id} onClick={() => setCharacterId((current) => current === entry.id ? "" : entry.id)} aria-pressed={characterId === entry.id} className={characterId === entry.id ? "active" : ""}><LibraryCharacterPortrait src={entry.portraitUrl} name={entry.name}/><span>{entry.name}</span><small>{count}</small></button>; })}{characterId && <button className="library-clear-character" onClick={() => setCharacterId("")}>Clear character</button>}</div>
    ...
    {bulkMode && <div className="library-bulk-bar"><span>{selectedIds.length} selected{bulkCollectionId ? ` · ${data.collections.find((entry) => entry.id === bulkCollectionId)?.name}` : ""}</span><button aria-label="Favorite" disabled={!selectedIds.length || busy} onClick={() => performBulk("favorite")}>Favorite</button><button aria-label="Remove Favorite" disabled={!selectedIds.length || busy} onClick={() => performBulk("unfavorite")}><span className="library-mobile-action-label">Remove</span><span className="library-desktop-action-label">Remove Favorite</span></button><button aria-label="Add as Reference" disabled={!selectedIds.length || busy} onClick={() => performBulk("reference")}><span className="library-mobile-action-label">Reference</span><span className="library-desktop-action-label">Add as Reference</span></button>{bulkCollectionId ? <button aria-label="Add to collection" disabled={!selectedIds.length || busy} onClick={() => performBulk("collection-add", bulkCollectionId)}><span className="library-mobile-action-label">Add</span><span className="library-desktop-action-label">Add to collection</span></button> : <select aria-label="Add selected to collection" defaultValue="" onChange={(event) => event.target.value && performBulk("collection-add", event.target.value)}><option value="">Collection</option>{data.collections.map((entry) => <option key={entry.id} value={entry.id}>{entry.name}</option>)}</select>}</div>}

    {openCollections && !selectedCollection && visibleCollections.length > 0 && <div className="library-collection-grid">{visibleCollections.map((entry) => { const assets = entry.mediaIds.map((id) => data.media.find((asset) => asset.id === id)).filter((asset): asset is MediaAsset => Boolean(asset)); const owners = [...new Set(assets.map((asset) => data.characters.find((characterEntry) => characterEntry.id === asset.characterId)?.name).filter(Boolean))]; const coverLayout = assets.length === 1 ? "is-single" : assets.length === 2 ? "is-double" : assets.length === 3 ? "is-triple" : ""; return <button className="library-collection-card" key={entry.id} onClick={() => setSelectedCollection(entry.id)}><span className={`library-collection-cover ${coverLayout}`}>{assets.length ? assets.slice(0, 4).map((asset) => <LibraryMediaThumbnail key={asset.id} asset={asset} className="library-collection-cover-item" loading="lazy"/> ) : <span className="library-cover-empty">◇</span>}<span className="library-collection-cover-shade" aria-hidden="true"/></span><span className="library-collection-copy"><strong>{entry.name}</strong><small>{assets.length} {assets.length === 1 ? "item" : "items"}{owners.length ? ` · ${owners.slice(0, 2).join(", ")}` : ""}</small></span></button>; })}</div>}
    ...
    {!emptyLibrary && !openCollections && media.length > 0 && <div className="library-grid" aria-label="Media archive">{media.map((asset, index) => { const owner = data.characters.find((entry) => entry.id === asset.characterId); const source = asset.providerId === "local-import" ? "Imported" : "Generated"; const assetRoles = roles.get(asset.id) ?? []; const selected = selectedIds.includes(asset.id); return <button key={asset.id} className={`library-tile ${selected ? "selected" : ""}`} onClick={() => bulkMode ? toggleSelected(asset.id) : select(asset)} aria-label={`${bulkMode ? selected ? "Deselect" : "Select" : "Open"} ${asset.title}`} aria-pressed={bulkMode ? selected : undefined}>
      <LibraryMediaThumbnail asset={asset} className="library-thumb" alt={asset.title} loading={index < 8 ? "eager" : "lazy"} style={{ aspectRatio: aspect(asset) }} />
    ...
    {selectedCollection && <div className="library-collection-items">{collectionItems.length === 0 ? <p>This collection is empty. Select assets in your Library and add them here.</p> : collectionItems.map((asset) => <div key={asset.id} className="library-collection-item"><button className="library-collection-item-open" onClick={() => select(asset)}><LibraryMediaThumbnail asset={asset} className="library-collection-item-thumb" loading="lazy"/><span>{asset.title}</span></button><button className="library-remove-item" aria-label={`Remove ${asset.title} from collection`} onClick={async () => { await runAction("collection-remove", { collectionId: selectedCollection, mediaId: asset.id }); await refresh(); }}>Remove</button></div>)}</div>}
