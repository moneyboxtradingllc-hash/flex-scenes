...
export function GenerationResult({ data, job, media, favorite, reference, addToCollection, create, openDetails, conversation }: {
  ...
  return <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_340px]">
    ...
    <aside className="rounded-[28px] border border-white/[.07] bg-[#12151b] p-5 sm:p-6">
      <div className="flex items-center gap-3"><span className="h-12 w-12 overflow-hidden rounded-full ring-2 ring-fuchsia-300/50"><LibraryCharacterPortrait src={character?.portraitUrl ?? ""} name={character?.name ?? ""} /></span><div className="min-w-0"><h2 className="truncate font-bold">{character?.name ?? "Scene"}</h2><p className="text-xs text-zinc-500">{new Date(media.createdAt).toLocaleString()}</p></div></div>
