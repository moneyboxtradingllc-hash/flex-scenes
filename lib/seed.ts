...
export function seedDatabase() {
  ...
  if(count) { ensureCanonicalFixtureReferences(); return; }
  const store=new LocalMediaStore(); const now=new Date().toISOString();
  const chars=[
  ...
  for(const [id,name,handle,description,personality,identityNotes,accent] of chars) { const portrait=store.ensureFixture(`${id}-portrait`,name,accent,"CHARACTER PORTRAIT"); db.prepare("INSERT INTO characters VALUES(?,?,?,?,?,?,?,?,?)").run(id,name,handle,portrait,description,personality,identityNotes,JSON.stringify({aspectRatio:"4:5",preset:"Hero"}),now); }
  const scenes=[
  ...
  for(const [characterId,title,caption,prompt,accent,type] of scenes) { const id=randomUUID(); const url=store.ensureFixture(`scene-${id}`,title,accent,type==="video"?"VIDEO PREVIEW":"EDITORIAL STILL"); db.prepare("INSERT INTO media VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)").run(id,characterId,type,url,type==="video"?url:null,title,caption,prompt,`fixture-${type}`,JSON.stringify({aspectRatio:"4:5"}),null,0,now,title==="After Hours Atrium"?1:0); }
