...
export async function POST(request:Request) {
  ...
  const roleValue=String(form.get("referenceRole")??""); const roles=["face","body","hair","look","outfit","environment","pose","motion","video","other"] as const; const role=roles.find((item)=>item===roleValue);
  ...
  try { if(role) repository.saveMediaAsCharacterReference(asset,role,form.get("canonical")==="true"); else repository.saveMedia(asset); }
  catch(error) { new LocalMediaStore().removeUpload(url); throw error; }
