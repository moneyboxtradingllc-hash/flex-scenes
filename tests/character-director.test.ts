...
function createTempCharacter(){const id=`test-director-${randomUUID()}`;const convo=`${id}-conversation`;const now=new Date().toISOString();db.prepare("INSERT INTO characters(id,name,handle,portraitUrl,description,personality,identityNotes,defaultsJson,createdAt,archived) VALUES(?,?,?,?,?,?,?,?,?,0)").run(id,"Test Director","@test","/qa-fixtures/test.png","Temporary test character","Test personality","Test identity","{}",now);db.prepare("INSERT INTO conversations VALUES(?,?,?,?)").run(convo,id,now,0);const profile=fixtureCharacterProfile(id);profile.initiativeLevel="DIRECTOR";repository.saveCharacterProfile(profile);repository.saveCreativeMemory(emptyCreativeMemory(id));repository.saveInitiativeState({characterId:id,lastEvaluatedAt:null,nextEligibleAt:null,lastProactiveProposalAt:null,dailyCount:0,dailyCountDate:now.slice(0,10),cooldownSeconds:28800,maxPerDay:1});return {id,convo};}
...
describe("Character Director M1 local brain",()=>{
  ...
  it("applies M1 migrations to clean and pre-M1 databases without dropping existing rows",()=>{
    const clean=migrationDatabase();clean.exec(migration("001_character_director.sql"));clean.exec(migration("002_separate_creative_and_conversation_profiles.sql"));clean.exec(migration("003_character_brain_foreign_keys.sql"));clean.exec(migration("004_character_roster_archive.sql"));clean.exec(migration("001_character_director.sql"));
    ...
    expect(saved.id).toBe("keep-character");expect(creative.speakingStyle).toBeUndefined();expect(creative.favoriteEnvironments).toEqual(["studio"]);expect((existing.prepare("SELECT archived FROM characters WHERE id='keep-character'").get() as {archived:number}).archived).toBe(0);
