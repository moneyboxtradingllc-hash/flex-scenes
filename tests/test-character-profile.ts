...
export function fixtureCharacterProfile(characterId: string): CharacterProfile {
  ...
  const defaults = emptyCharacterProfile(characterId);
  const shared = {
    ...defaults.conversationalProfile,
    confidence: 0.72,
  ...
  return profiles[characterId] ?? {
    ...
    creativeProfile: { ...defaults.creativeProfile, favoriteEnvironments: ["studio"], wardrobeCategories: ["editorial"], preferredLighting: ["soft natural light"], preferredMoods: ["calm"], preferredShotTypes: ["portrait"], cameraEnergy: ["gentle push-in"], mediaBalance: "balanced", experimentationLevel: 0.5, visualThemes: [], avoidedThemes: [], ideasToTry: [], ideasTiredOf: [], creativeBoldness: 0.5, noveltyPreference: 0.5, repetitionTolerance: 0.5 },
    updatedAt: now,
  };
}
